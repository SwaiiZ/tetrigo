...
func (m *AIModel) gameOverUpdate(msg tea.Msg) (*AIModel, tea.Cmd) {
...
func (m *AIModel) pausedUpdate(msg tea.Msg) (*AIModel, tea.Cmd) {
	if msg, ok := msg.(tea.KeyMsg); ok {
		switch {
...
func (m *AIModel) playingUpdate(msg tea.Msg) (*AIModel, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
...
func (m *AIModel) FindBestPlacement(mat tetris.Matrix, t tetris.Tetrimino) []Placement {
	...
	resultCh := make(chan []Placement, rotationCount)

	for rot := 0; rot < rotationCount; rot++ {
		rot := rot // avoid closure issue
		go func() {
	...
	for i := 0; i < rotationCount; i++ {
		allPlacements = append(allPlacements, <-resultCh...)
	}
	...
	for _, move := range candidates {
		...
		go func() {
			...
			resultCh <- struct {
				seq   []Placement
				score float64
			}{append([]Placement{move}, seq...), total}
		}()
	}

	bestScore := math.Inf(-1)
	var bestSeq []Placement
...
func (m *AIModel) matrixView() (string, error) {
...
func (m *AIModel) informationView() string {
...
func (m *AIModel) holdView() string {
...
func (m *AIModel) renderTetrimino(t *tetris.Tetrimino, background byte) string {
	var output string
...
func (m *AIModel) renderCell(cell byte) string {
	switch cell {
	case 0:
...
func (m *AIModel) triggerGameOver() tea.Cmd {
	m.game.EndGame()
	m.isPaused = false
...
func (m *AIModel) togglePause() tea.Cmd {
